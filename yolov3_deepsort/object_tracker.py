...
flags.DEFINE_string('video', './data/video/despacho.mp4',
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if class_name == human:
                ...
                nonhuman_index = []
                mainCenterPoint = (int((int(bbox[2]) - int(bbox[0]))/2 + int(bbox[0])), int((int(bbox[3])-int(bbox[1]))/2 + int(bbox[1])))
                centerPoints = []
                lengths = []
                diagonals = []
                for track in tracker.tracks:
                    ...
                    centerPoint = (int((int(bbox[2]) - int(bbox[0]))/2 + int(bbox[0])), int((int(bbox[3])-int(bbox[1]))/2 + int(bbox[1])))
                    length = math.sqrt(((mainCenterPoint[0] - centerPoint[0]))**2 + (mainCenterPoint[1] - centerPoint[1])**2)
                ...
                if not (diagonals == []):
                    cp_diagonals = list(diagonals) #creamos una copia del arreglo de diagonales
                    cp_diagonals.sort()
